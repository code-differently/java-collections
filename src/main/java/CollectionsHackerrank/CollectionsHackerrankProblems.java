    ...
    public static Integer mostFrequent(List<Integer> numbers) {
        ...
        for (Integer num : numbers) {
            if (numberCount.containsKey(num)) {
                int value = numberCount.get(num);
                numberCount.replace(num, value + 1);
            } else {
        ...
        for (Integer key : numberCount.keySet()) {
            int currentValue = numberCount.get(key);
            if (numberCount.get(key) > max) {
                max = currentValue;
                answer = key;
            }
        }
    ...
    */
    public static Map<Integer, List<String>> groupByLength(List<String> words) {
        ...
        for (String word : words) {
            ...
            if (answer.containsKey(length)) {
                List<String> update = answer.get(length);
                update.add(word);
                answer.replace(length, update);
            } else {
                answer.put(length, new ArrayList<>(List.of(word)));
            }
        }
        return answer;
    }
